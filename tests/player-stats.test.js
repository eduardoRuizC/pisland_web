...
test("every team uses the new stats and versioned public data URLs", async () => {
  ...
  assert.match(indexMarkup, /data-manifest-url="teams\/index\.json\?v=12"/u);
  assert.ok(manifest.teams.every((filename) => /\.json\?v=\d+$/u.test(filename)));
  assert.ok(manifest.teams.includes("rompediscotecas.json?v=7"));
  assert.ok(manifest.teams.includes("gargolas.json?v=7"));
  assert.ok(manifest.teams.includes("bichotas.json?v=7"));
  assert.ok(manifest.teams.includes("sangre-nueva.json?v=7"));
